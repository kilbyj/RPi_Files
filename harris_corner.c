...
int main()
{
    ...
    int fd;
    long int screensize = 0,row_size = 0, col_size = 0;
    ...
    FD_ZERO(&fds);
    FD_SET(fd_cap, &fds);
    unsigned short gray_image[MAX_HEIGHT][MAX_WIDTH];
    while (1) {
		...
		//gray_image[offset+2] = ((((r[1]>>3))<<11) | (((g[1]>>2))<<5) | ((b[1]>>3)));
                //*((unsigned short*)(fbp + offset)) = ((((r[0]>>3))<<11) | (((g[0]>>2))<<5) | ((b[0]>>3)));
                
                //*((unsigned short*)(fbp + offset + 2)) = ((((r[1]>>3))<<11) | (((g[1]>>2))<<5) | ((b[1]>>3)));
            }
        }
	...
	//display the image for testing right now later we will draw image everywhere until we find one greater than our threshold and then at that pixel we will draw a bright RED dot
	for(i=0;i<480;i++){
		for(j=0;j<640;j++){
			offset = (j+vinfo.xoffset)*(vinfo.bits_per_pixel/8) + (i+vinfo.yoffset)*finfo.line_length;
    ...
    //setup the webcam
    int fd_cap,ret,y0,u,y1,v,r[2],g[2],b[2],offset;
